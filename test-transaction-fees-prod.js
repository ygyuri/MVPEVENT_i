  ...
  try {
    ...
    for (const test of testCases) {
      ...
      console.log(
        `${status} ${test.amount
          .toLocaleString()
          .padEnd(10)} → Fee: ${result.fee
          .toString()
          .padEnd(5)} KES (Expected: ${test.expected}) [${
          result.feeDetails?.tierName || "N/A"
        }] - ${test.desc}`
      );
    }
    ...
    if (orders.length > 0) {
      ...
      orders.forEach((order, i) => {
        ...
        console.log(
          `  Subtotal: ${(order.pricing?.subtotal || 0).toLocaleString()} ${
        ...
        console.log(
          ...
          ).toLocaleString()} ${order.pricing?.currency || "KES"}`
        );
        if (order.pricing?.transactionFeeDetails) {
          console.log(
            `  Fee Tier: ${order.pricing.transactionFeeDetails.tierName}`
          );
        }
        console.log(
          `  Total: ${(
        ...
        console.log(
          `  Status: ${order.paymentStatus || order.status || "N/A"}\n`
        );
      });
    } else {
      console.log("⚠️  No orders found with transaction fee field");
      console.log(
        "   This is normal if no orders were created after the feature was added.\n"
      );
    }
