...
const EventList = ({
  ...
  // Handle event action
  const handleEventAction = useCallback((action, eventId, eventData) => {
    onEventAction?.(action, eventId, eventData);
    setActionMenuOpen(null);
  }, [onEventAction]);
  
  // Toggle event expansion
  const toggleExpanded = useCallback((eventId) => {
    setExpandedEvent(expandedEvent === eventId ? null : eventId);
  }, [expandedEvent]);
  
  // Toggle action menu
  const toggleActionMenu = useCallback((eventId) => {
    ...
    return (
      <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-12 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6">
          Get started by creating your first event or adjust your filters to see more results.
        </p>
        <button
      ...
      <div className="hidden lg:block bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 overflow-x-auto">
        <div className="grid grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-1">
            <button
              onClick={() => handleSelectAll(selectedEvents.length !== events.length)}
          ...
          {sortHeaders.map((header) => (
            <div key={header.key} className={`${header.key === 'title' ? 'col-span-3' : 'col-span-2'}`}>
              <button
                onClick={() => header.sortable && handleSortChange(header.key)}
                className={`flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 ${
                  header.sortable ? 'hover:text-gray-900 dark:hover:text-white cursor-pointer' : 'cursor-default'
                }`}
              >
                {header.label}
      ...
      <div className="space-y-4">
        {events.map((event) => (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="bg-web3-card rounded-xl p-6 hover:bg-web3-card-hover transition-all duration-200 border border-gray-200 dark:border-gray-700"
          >
            {/* Mobile Layout */}
            <div className="lg:hidden">
              <div className="flex items-start gap-4">
                {/* Selection Checkbox */}
                <button
                  onClick={() => handleEventSelect(event._id, !selectedEvents.includes(event._id))}
