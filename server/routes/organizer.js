...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (mimetype && extname) {
      ...
      cb(new Error('Only image files (JPEG, PNG, GIF, WebP, SVG, BMP, TIFF, PDF) are allowed'));
    }
  }
});
...
const processBase64Images = async (media, eventId) => {
  ...
  if (media.coverImageUrl && media.coverImageUrl.startsWith('data:image/')) {
    try {
      const imageUrl = await saveBase64Image(media.coverImageUrl, eventId, 'cover');
      processedMedia.coverImageUrl = imageUrl;
      console.log('✅ [BASE64 PROCESSING] Cover image saved:', imageUrl);
    } catch (error) {
      console.error('❌ [BASE64 PROCESSING] Failed to save cover image:', error);
    }
  }
    ...
    for (let i = 0; i < media.galleryUrls.length; i++) {
      ...
      if (url && url.startsWith('data:image/')) {
        try {
          ...
          console.error('❌ [BASE64 PROCESSING] Failed to save gallery image:', error);
        }
      } else {
...
const addDays = (date, days) => new Date(new Date(date).getTime() + days * msPerDay);
...
const generateOccurrences = (master, rule) => {
  ...
  if (rule.frequency === 'daily') {
    ...
    const weekdays = Array.isArray(rule.byWeekday) && rule.byWeekday.length ? rule.byWeekday.slice().sort() : [new Date(start).getDay()];
    ...
    while (generated < max) {
      for (const wd of weekdays) {
        ...
        if (until && occStart > until) { generated = max; break; }
...
// List my events
router.get('/events', verifyToken, requireRole(['organizer','admin']), [
  ...
  try {
    ...
    res.json({ success: true, data: { items, pagination: { page: Number(page), pageSize: Number(pageSize), total, totalPages } } });
  } catch (error) {
...
router.get('/events/:id', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId()
], async (req, res) => {
  try {
    if (!handleValidation(req, res)) return;
...
// Create draft
router.post('/events', verifyToken, requireRole(['organizer','admin']), [
  ...
  try {
    ...
    try {
      ...
      console.log('🧪 [CREATE EVENT] Schema.tags caster:', tagsPath?.caster?.instance, 'options:', tagsPath?.options);
    } catch (_) {}
...
router.patch('/events/:id', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId(),
  ...
  try {
    ...
    const owns = ensureOwnership(event, req.user);
    if (!owns.ok) return res.status(owns.code).json({ error: owns.msg });
    if (event.status !== 'draft') return res.status(400).json({ error: 'Only drafts can be updated' });
    if (typeof req.body.version === 'number' && event.version !== req.body.version) {
      return res.status(409).json({ error: 'Version conflict. Refresh and retry.' });
    }
    const updatable = ['title','description','shortDescription','category','location','dates','capacity','pricing','flags','media','ticketTypes','tags','metadata','qrSettings','recurrence'];
    for (const key of updatable) {
      if (req.body[key] !== undefined) {
        if (key === 'tags') {
          const before = req.body[key];
          event[key] = normalizeTags(req.body[key]);
          console.log('🏷️ [TAGS][PATCH] Normalized:', { before, after: event[key] });
        } else {
    ...
    await event.save();
    res.json({ success: true, data: { id: event._id, updatedAt: event.updatedAt, version: event.version } });
  } catch (error) {
...
router.put('/events/:id/tickets', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId(),
  body('ticketTypes').isArray({ min: 0 })
], async (req, res) => {
  try {
    if (!handleValidation(req, res)) return;
    const event = await Event.findById(req.params.id);
    ...
    for (const t of ticketTypes) {
      if (t.price !== undefined && t.price < 0) return res.status(400).json({ error: 'Ticket price must be >= 0' });
      if (t.quantity !== undefined && t.quantity < 0) return res.status(400).json({ error: 'Ticket quantity must be >= 0' });
      if (t.salesStart && t.salesEnd && new Date(t.salesStart) > new Date(t.salesEnd)) {
        return res.status(400).json({ error: 'Ticket salesStart must be before salesEnd' });
      }
    }
    const capacity = event.capacity || Infinity;
    const totalQty = ticketTypes.reduce((sum, t) => sum + (Number(t.quantity) || 0), 0);
    if (capacity !== Infinity && totalQty > capacity) return res.status(400).json({ error: 'Sum of ticket quantities exceeds capacity' });
...
router.post('/events/:id/publish', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId()
], async (req, res) => {
  try {
    if (!handleValidation(req, res)) return;
    ...
    const owns = ensureOwnership(event, req.user);
    if (!owns.ok) return res.status(owns.code).json({ error: owns.msg });
    if (event.status !== 'draft') return res.status(400).json({ error: 'Only drafts can be published' });
    ...
    if (!event.title || !event.description) return res.status(400).json({ error: 'Missing required fields: title/description' });
    const ds = event.dates || {};
    if (!ds.startDate || !ds.endDate) return res.status(400).json({ error: 'Missing required fields: dates.startDate/endDate' });
    if (new Date(ds.startDate) >= new Date(ds.endDate)) return res.status(400).json({ error: 'startDate must be before endDate' });
    if (event.pricing && event.pricing.isFree === false && (event.pricing.price || 0) < 0) {
      return res.status(400).json({ error: 'Invalid pricing.price' });
    }
    
    // Process base64 images if present
    if (event.media) {
      const processedMedia = await processBase64Images(event.media, event._id);
    ...
    if (rec.enabled) {
      ...
      for (const occ of occs) {
        const occSlugBase = `${event.slug}-${occ.start.toISOString().slice(0,10)}`;
        ...
        const child = {
          ...
          dates: { startDate: occ.start, endDate: occ.end, timezone: event.dates?.timezone || 'UTC' },
      ...
      await event.save();
      return res.json({ success: true, data: { masterId: event._id, occurrences: inserted.map(x => ({ id: x._id, slug: x.slug, startDate: x.dates.startDate })) } });
    } else {
      event.status = 'published';
      await event.save();
      ...
      try {
        ...
        if (organizer) {
          ...
          await emailService.sendEventPublishedAdminNotification(event, organizer);
          
          console.log('✅ Email notifications sent for published event:', event._id);
        }
      } catch (emailError) {
        console.warn('⚠️ Failed to send email notifications:', emailError.message);
        // Don't fail the publish if emails fail
      }
      
      return res.json({ success: true, data: { id: event._id, slug: event.slug } });
    }
  } catch (error) {
    if (error && error.code === 11000) {
      return res.status(409).json({ error: 'Slug already exists. Try changing the title.' });
    }
...
// Cancel
router.post('/events/:id/cancel', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId()
], async (req, res) => {
  try {
    if (!handleValidation(req, res)) return;
...
router.post('/events/:id/unpublish', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId()
], async (req, res) => {
  try {
    if (!handleValidation(req, res)) return;
    ...
    const owns = ensureOwnership(event, req.user);
    if (!owns.ok) return res.status(owns.code).json({ error: owns.msg });
    if (event.status !== 'published') return res.status(400).json({ error: 'Only published events can be unpublished' });
    const now = new Date();
    if (event.dates?.startDate && new Date(event.dates.startDate) <= now) {
      return res.status(400).json({ error: 'Cannot unpublish events that already started' });
    }
    event.status = 'draft';
    ...
    event.version = (event.version || 0) + 1;
    await event.save();
    res.json({ success: true, data: { id: event._id, status: event.status, version: event.version } });
  } catch (error) {
...
// Upload images
router.post('/events/:id/upload', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId()
...
// Clone
router.post('/events/:id/clone', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId(),
  ...
  try {
    ...
    if (!owns.ok) return res.status(owns.code).json({ error: owns.msg });
    const shiftDaysNum = typeof req.body.shiftDays === 'number' ? req.body.shiftDays : null;
    ...
    if (shiftDaysNum !== null && newDates?.startDate && newDates?.endDate) {
      newDates.startDate = addDays(new Date(src.dates.startDate), shiftDaysNum);
...
// Delete event
router.delete('/events/:id', verifyToken, requireRole(['organizer','admin']), [
  param('id').isMongoId()
], async (req, res) => {
  try {
    if (!handleValidation(req, res)) return;
    ...
    if (hasOrders > 0) {
      return res.status(400).json({ 
        error: 'Cannot delete event with existing orders. Cancel the event instead.' 
      });
    }
